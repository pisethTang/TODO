...
const User = {
    ...
    create: async (firstName, lastName, email, passwordHash) => {
        let sqlString = "INSERT INTO users (first_name, last_name, email, password_hash) VALUES (?, ?, ?, ?)";
        const [results] = await pool.query(sqlString, [firstName, lastName, email, passwordHash]);
